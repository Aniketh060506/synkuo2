...
const NATIVE_APP_NAME = 'com.synkup.app';

console.log('[Background] Service worker started - initially disconnected');
...
function connectNativeApp() {
    try {
        ...
        isConnected = false;
        targetNotebook = { id: 'default', name: 'Connecting...' };
        ...
        nativePort.onDisconnect.addListener(() => {
            ...
            // Try to reconnect after 5 seconds only if not manually disconnected
        ...
        // Send ping to verify connection (with timeout)
        setTimeout(() => {
            if (nativePort && !isConnected) {
                console.log('[Background] Connection timeout - marking as disconnected');
                isConnected = false;
                targetNotebook = { id: 'default', name: 'Desktop App Not Running' };
            }
        }, 3000);
    ...
    switch (message.type) {
        ...
        case 'CAPTURE_CONTENT':
            if (isConnected) {
                ...
                sendResponse({
                    success: false,
                    message: 'Desktop app not connected. Please open SynkUp desktop app.'
                });
            }
            break;
